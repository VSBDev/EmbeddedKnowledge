...
**What reversing the fit does and does not test.** Fit the line the other way, predicting the interval from glucose. Least squares returns a slope of −0.0304 hours per mg/dL. Turn that upside down to put it back in the first fit's units and it gives −32.9 mg/dL per hour, nearly ten times as steep as the −3.37 from predicting glucose from interval.

Both lines are correct least-squares fits to the same sixty dots. They answer different conditional-mean questions and minimise errors in different directions, so ordinary least squares is asymmetric and the analyst must choose the prediction direction from the question. Multiply the two slopes and you get 0.102, which is {math}`r^2` exactly. That algebra is useful for understanding the fit, but it is not a test of causation or mechanism: even a genuine directional causal relation can be noisy, and reversing the response and predictor still asks a different question.

**The better model.** The slope is a statement about *association per hour*: across these sixty adults, the ones with a one-hour longer interval averaged 3.37 mg/dL lower fasting glucose. This was an observational comparison between different people, with uncontrolled or poorly measured rival explanations, no intervention, and no longitudinal contrast within people. Those design limits are why it cannot identify an effect of changing dinner time. The earlier lesson on [correlation and causation](https://embeddedknowledge.io/premed/lessons/read/?lesson=PREM-SCI-007) sets out what an identified causal design requires; changing which variable sits on the left of an ordinary least-squares equation supplies none of it.
:::
...
Which makes this lesson's own fit an instructive case in the opposite corner. Its {math}`r^2` is 0.10, near the bottom of anything a journal would print, and its residual plot shows no clear arch, funnel, or single point dragging the line. Over the observed range, this plot provides no clear evidence against the straight-line form; it does not prove that form true or uniquely correct, especially with a sample of sixty and only the diagnostics examined here. The observed linear relationship is weak. Those are separate findings, and the second question above has no answer without seeing the other team's residual plot.
...
:::{source-note}
:claims: claim-r-squared-not-sufficient, claim-residual-plot-structure, claim-least-squares-outlier-sensitivity, claim-least-squares-criterion
:sources: source-nist-model-fit, source-nist-residual-structure, source-nist-least-squares, source-kim-regression-basics

These sources support the statements that least squares minimises deviations in the chosen response direction, a high coefficient of determination does not guarantee that a model fits the data well, graphical residual analysis is the primary means of judging fit while no single residual plot can justify a model on its own, systematic residual structure indicates that the functional form can be improved, and least-squares fits are highly sensitive to outliers. The reverse fit, the nine constructed values, and both laboratory reports were written for this lesson.
:::
