...
The fourth is worth a pause, and it exposes an ambiguity in the question itself. Two different things can be classified here and this lesson's test only settles one of them.

Ask about the **event** — this glucose molecule, this crossing — and the two-part test answers plainly: the glucose is running downhill, so nothing about the glucose required energy from outside its own gradient. By the event test, that crossing is not doing uphill work.

Ask about the **protein** and the answer is different and equally correct. SGLT1 is a secondary active cotransporter. That names what it is built to do and how it is powered when it does it, and it does not change with the luminal concentration.

The coupling is what connects them. Two sodium ions come in with every glucose molecule whether or not the glucose needed the help, so sodium is running down its own gradient either way and the store is being spent either way. And the cycle is reversible: which way SGLT1 runs is set by the sodium and glucose gradients together, not fixed by the protein's name. So the honest form of the answer is to say which question you are answering. Naming the protein is a claim about mechanism; naming the crossing is a claim about this event's energetics. Early in a meal, with the lumen concentrated, glucose is running downhill and the coupling is buying nothing it could not have had for free. Later, when the lumen has been stripped, the same protein is driving glucose uphill and the coupling is the only reason it can.
