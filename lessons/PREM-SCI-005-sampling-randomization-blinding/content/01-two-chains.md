...
The second is about **how the two groups were formed and what people knew**. If the calmer-looking volunteers ended up in the breathing group, or if students who knew they had been given "the real thing" rated themselves generously, then the 2.1 partly records those processes instead of breathing.
...
Does the description above actually tell you who decided which students did the breathing routine and which sat quietly? Check before you answer. Then consider each possibility in turn, whether the research team decided or the students decided, and say what each one would change about the conclusion the team can draw.
:::

It does not tell you. The opening says only that some students did the routine and others sat quietly: a description of what happened, not of who chose it. Noticing that a method description is silent is itself part of the skill this lesson builds.
...
Walk through a study in time. The audit has four questions, but they are **not four separate events**. There are three broad phases: entry, one allocation event, and follow-up, with two of the questions inspecting different properties of that same allocation event.
...
**At allocation, ask what determined the assignment.** A study may generate a chance-based sequence before recruitment begins, or it may use a chance procedure only after a person has enrolled. Either way, allocation and assignment name the same participant-level event: the person is placed in one condition or the other. Call this the assignment question: *what rule and chance process determined the group?*
...
**Afterwards, the study runs and outcomes are measured.** Now assignment has happened, and the question changes from who gets what to who *knows* what. Call this the awareness question: *after assignment, who knew which condition each person received?*
...
The rules about who could enter run before anyone is assigned to anything. Because of that, they cannot make the two groups differ from each other, since both groups are drawn from exactly the same pool. What they do instead is decide which population the finding is about.
...
**Random allocation** means that, before a participant is assigned, a prespecified procedure defines the possible assignments and their probabilities, and a chance process selects among them. The simplest model in this lesson is **simple randomization**: every participant faces the same allocation ratio, much like a properly implemented coin-toss rule.

Simple randomization ignores participant characteristics, but not every valid random allocation method does. **Restricted randomization** may use blocks to keep group sizes close. **Stratified randomization** may first group participants by centre, age band, or disease severity and then randomize within each stratum. Covariate-adaptive methods may change assignment probabilities in response to earlier allocations while retaining a chance component. Minimisation can also use selected baseline factors; it may be deterministic or may add a random component, so its exact procedure must be reported, not assumed. Using a characteristic to define a stratum is compatible with random allocation because chance still determines the assignment within the prespecified rule. Restricted methods can nevertheless make a later assignment easier to guess from earlier ones, especially near the end of a fixed block. That operational predictability is a reason to protect concealment; it does not turn a chance-generated sequence into a deterministic one.

Sequence generation and concealment do different jobs. The chance component removes deterministic investigator choice when the prespecified sequence is generated and followed, and it supports a comparison that is fair *in expectation*, including for factors nobody measured or anticipated. By itself, however, chance generation does not stop an enroller from pairing a particular participant with an upcoming assignment they know or can guess. Concealment and faithful implementation block that manipulation. Stratification can deliberately balance selected measured factors, but it does not replace either the chance component's protection against discretionary assignment or concealment's protection against foreknowledge.
...
- Schemes that look arbitrary are not thereby random. Alternation, date of birth, and hospital record number are deterministic, so they do not provide randomization-based protection. Their predictability also permits foreknowledge, manipulation, or systematic imbalance. But predictability is not the definition of random allocation: an upcoming assignment in a valid restricted random sequence can sometimes become guessable, which is a concealment vulnerability rather than proof that chance generation failed. A deterministic scheme does **not** prove that every realized comparison is biased; it means the design gives no randomization-based reason to rule that bias out. Reporting standards ask authors not to call these methods random.
...
Steering people into the group you prefer, at the point of entry, is a form of **selection bias.** This is the general name for a distortion produced by how people came to be in the groups being compared, distinct from anything done to them afterwards. Concealment exists to shut that route.
...
One practical consequence is worth remembering within the individually randomized, person-by-person setting taught here: concealment can be arranged because researchers control the order of commitment and reveal. Do not export that "always arrangeable" rule to a design in which allocation occurs before individual recruitment; such a design needs its own account of which decision remains open and who can act on foreknowledge.
...
The party at the end of that list is the least obvious and worth a sentence of its own. Analysis involves choices about which participants to include when some records are incomplete, where to draw a cut-off, and which comparison to report first. A statistician who knows which column is the treatment can make each of those small choices in the direction they hope for, again without any intent to deceive. Handing over the data with the groups labelled only A and B removes that pressure, and it costs nothing.
