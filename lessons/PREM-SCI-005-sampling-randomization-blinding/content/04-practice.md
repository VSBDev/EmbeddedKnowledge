...
*Assignment.* A shuffled deck is a genuine chance device, so this is not alternation or a birth-date rule, and the assignment does not depend on anything about the player. Assignment within club is a reasonable choice, since it prevents club-level differences from lining up with the arms.
...
*Strongest conclusion and boundary.* Very little is warranted. Two independent routes could produce a difference in strain rates without the routine doing anything, and one of them corrupts who was compared while the other corrupts what was recorded. The honest statement is that this study cannot distinguish an effect of the warm-up from selection at assignment plus outcome judgement. The boundary on even a repaired version would remain the eleven self-selecting clubs.
...
The interesting failure is in awareness, and it is a subtle one that the identical appearance is designed to hide. The tablets look the same, so blinding appears intact — but taste breaks it. A participant who tastes something bitter can infer they are on the supplement, which means participant blinding fails in practice despite a well-designed placebo. Because the outcome is a self-scored fatigue questionnaire, that inference feeds directly into the measurement: the person who has guessed their assignment is the person generating the outcome data. The blinded nurse does not rescue this, because the nurse is only reviewing numbers the participant already produced. Blinding an assessor protects against the assessor's expectations, not the participant's.

The strongest warranted claim is weak: among self-selected respondents to a fatigue advertisement, those assigned the supplement reported less fatigue, in a trial with sound allocation and concealment but with participant blinding likely defeated by taste on an entirely self-reported outcome. The material boundary is that expectation among participants who could identify their assignment is a sufficient alternative explanation, so the difference cannot be attributed to the mineral.

If you want to check the general lesson here: a placebo control is not the same thing as successful blinding. Blinding succeeded only if participants could not tell, and that is an empirical question about this particular placebo — which is why careful trials sometimes ask participants at the end which group they believed they were in.
...
:::{source-note}
:claims: claim-entry-bounds-applicability, claim-sampling-representativeness, claim-random-allocation-comparability, claim-concealment-definition, claim-concealment-blinding-distinct, claim-blinding-prevents, claim-blinding-fallback
:sources: source-consort-2010-elaboration, source-blinding-overview, source-sampling-participants

Each practice study applies the same sourced distinctions: which methods count as random assignment, what allocation concealment protects and when, which parties blinding covers and which biases it prevents, blinded outcome assessment as the repair when participants cannot be blinded, and the effect of a self-selected sampling route on who a result describes. The clinic, sports-science, and nutrition studies are fictional teaching cases written for this lesson; none reports a real trial, and none of them supports any conclusion about text reminders, warm-up routines, or mineral supplements.
:::
