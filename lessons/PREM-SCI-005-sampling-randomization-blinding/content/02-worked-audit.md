...
> **The breathing study (fictional teaching case).** A poster in the 24-hour study room invites students to join "a six-minute pre-exam calm study." Sixty students sign up over two weeks. An independent data officer generates the random sequence, prepares a stack of sixty sequentially numbered, opaque, sealed envelopes, and keeps the master sequence inaccessible to the research assistant. On the morning of the midterm, the assistant checks each participant's eligibility, obtains consent, irreversibly records the participant's acceptance and enrolment details, and only then opens the next envelope. The envelope says either *breathing* or *quiet sitting*. Next, the assistant delivers the assigned six minutes and, twenty minutes later, asks the participant to rate their calm from 0 to 10 and writes the rating on the participant's sheet. The breathing group averages 2.1 points higher.
...
**What we are given.** A recruitment route (poster in one room, self-selected volunteers). An assignment mechanism (computer-generated random sequence). A concealment mechanism (independently prepared and controlled sequentially numbered, opaque, sealed envelopes, opened only after consent and enrolment are recorded). An outcome procedure (the same assistant delivers the intervention and collects the self-rating). A result (2.1 points on a 0–10 self-report scale).
...
A computer-generated random sequence. This is a chance-based procedure, not alternation or a date rule. The description does not say whether the sequence was simple, blocked, or stratified, and a complete report should because restricted methods can differ in operational predictability. None gives the assistant discretion over the sequence when the prespecified assignments are faithfully followed. The two groups are therefore comparable in expectation. With sixty participants, that expectation is a weaker guarantee than it would be at six hundred: chance imbalance in a small trial is entirely possible, and the report should show what the groups actually looked like at baseline.

**Step 3. Foreknowledge: was each entry committed before the assignment could be known?**
Yes, and this part is done well. The independent data officer prepared and controls the sequence and numbered, opaque, sealed envelopes; the assistant cannot inspect the sequence and opens the next envelope only after consent, acceptance, and enrolment details are irreversibly recorded. Commitment comes first and knowability second: the assistant has to enrol this student before learning what the student will get, so there is no way to steer a distressed-looking student toward the breathing arm. Allocation concealment is intact, and because the independently held sequence exists in advance on file, the procedure is also auditable afterwards.

**Step 4. Awareness: after assignment, who knew what?**
State the **effect of interest** before judging this step. If the question is the total effect of being assigned the guided-breathing package instead of quiet sitting, then the attention, meaning, and ordinary expectations created by those two experiences are part of what was assigned. If the narrower question is whether the breathing content has an effect beyond attention and expectation, this comparator cannot answer it.
...
Then keep two criticisms of that sample apart, because they are easy to bundle and they belong to different steps. That the sample was *self-selected* is a step-1 point, and it limits who the answer is about. That the sample was *small* is a step-2 point, and it weakens how much comfort you take from "comparable in expectation." Neither is the fatal problem. They fail differently, and an answer that treats them as one thing is only half right.
:::
