...
A **negative control** deliberately substitutes an exposure or outcome that should not have the tested causal relation but should share important bias-producing conditions. This is a *negative control* in the causal-inference sense — a chosen comparison exposure or outcome — which differs from the laboratory negative control of [Variables and controls (PREM-SCI-003)](https://embeddedknowledge.io/premed/lessons/read/?lesson=PREM-SCI-003), a physical baseline condition expected to show no response. Both share the logic that an effect where none should appear signals a problem, but one is a chosen exposure and the other a bench condition. If the supposedly impossible association appears, residual confounding, selection, or measurement error may still be present. A clean negative control does not prove that every bias is absent; the control works only if its own assumptions are credible.
