...
The mastery check runs on tables you have not seen, so that nothing can be answered from memory of the study. Nine items, retakeable, feedback after the attempt. Work each with the relevant table in front of you and a pencil; nothing here needs software.
...
The first table supports five core items. A second invented natural-frequency table supports four screening-measure calculations. Together the nine items ask you to:
...
5. calculate sensitivity, specificity, positive predictive value, and negative predictive value from counts, naming the denominator of each;
6. decide what a run of like results does to the probability of the next independent one.
...
- **Within two weeks.** Reconstruct the four screening measures from an invented two-by-two count table: read sensitivity and specificity down the true-state columns, then positive and negative predictive values across the result rows.
