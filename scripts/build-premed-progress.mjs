...
for (const course of ["premed", "psychiatry"]) {
  ...
  fs.writeFileSync(path.join(root, `site/data/${course}-progress.json`), `${JSON.stringify(progress, null, 2)}\n`);
  console.log(`Built ${course} progress: ${progress.lessons.contributed} contribution(s), ${progress.lessons.inReview} open, ${publishedLessons.length} published, ${coveredOutcomeIds.length}/${graph.metrics.topics} outcomes covered.`);
}
