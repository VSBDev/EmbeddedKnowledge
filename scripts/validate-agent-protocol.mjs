...
import { eligibleReviewVerdicts, reviewInputMinimum, reviewVerdictCounts, validateFinalizationRecord } from "./lib/quorum-policy.mjs";
...
for (const tier of ["minor-correction", "standard", "high-impact"]) {
  ...
  if (roleTotal !== reviewInputMinimum(rule)) errors.push(`${tier}: role minimums do not equal minimum review inputs.`);
  if (rule.minimumDistinctAgentRuns !== reviewInputMinimum(rule)) errors.push(`${tier}: each review input must use a distinct agent run.`);
  if (rule.minimumApprovingReviews > reviewInputMinimum(rule)) errors.push(`${tier}: approval floor exceeds the review-input floor.`);
  if (rule.minimumDistinctPrincipals < 1 || rule.minimumDistinctPrincipals > reviewInputMinimum(rule)) errors.push(`${tier}: distinct-principal floor is invalid.`);
...
for (const review of reviews) {
  ...
  if (!eligibleReviewVerdicts(standard).has(review.verdict)) errors.push(`${review.reviewId}: verdict is ineligible for the standard finalization record.`);
...
if (reviews.length !== reviewInputMinimum(standard)) errors.push("Example review set does not satisfy the standard review-input count.");
...
if (reviewAgentRuns.size !== standard.minimumDistinctAgentRuns) errors.push("Example review set does not use the required distinct agent runs.");
...
const verdictCounts = reviewVerdictCounts(reviews);
if (adjudication.quorum.approvals !== verdictCounts.approvals || adjudication.quorum.reviewInputs !== verdictCounts.reviewInputs || adjudication.quorum.changeRequests !== verdictCounts.changeRequests || adjudication.quorum.distinctPrincipals !== reviewPrincipals.size || adjudication.quorum.distinctAgentRuns !== reviewAgentRuns.size) {
...
errors.push(...validateFinalizationRecord(adjudication, reviews, standard).map((problem) => `Example adjudication: ${problem}`));
