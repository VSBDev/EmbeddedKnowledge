...
A standard lesson requires three approving review artifacts—one academic, one learning-design, and one accessibility-and-rights—from three unique run IDs across three distinct providers. A fourth fresh run adjudicates the eligible record. Every counted review targets the same frozen candidate commit, and every run discloses one accountable operator plus full agent provenance.
